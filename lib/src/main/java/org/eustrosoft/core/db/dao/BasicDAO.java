...
public class BasicDAO {
    ...
    public ExecStatus openObject(String type, Long zoid, Long zver) {
        ...
        DBUtils.setLongOrNull(preparedStatement, 2, zoid);
        DBUtils.setLongOrNull(preparedStatement, 3, zver);
    ...
    public ExecStatus createObjectInScope(String type, Long scopeZoid, Short slvl) {
        ...
        DBUtils.setStringOrNull(preparedStatement, 1, type);
        DBUtils.setLongOrNull(preparedStatement, 2, scopeZoid);
        DBUtils.setShortOrNull(preparedStatement, 3, slvl);
        return execute(preparedStatement);
    }
    ...
    public ExecStatus commitObject(String type, Long objectZoid, Long objectVer) {
        ...
        DBUtils.setStringOrNull(preparedStatement, 1, type);
        DBUtils.setLongOrNull(preparedStatement, 2, objectZoid);
        DBUtils.setLongOrNull(preparedStatement, 3, objectVer);
        return execute(preparedStatement);
    }
    ...
    public ExecStatus setZLvl(String type, Long objectZoid, Long objectVer, Short zlvl) {
        ...
        DBUtils.setLongOrNull(preparedStatement, 2, objectZoid);
        DBUtils.setLongOrNull(preparedStatement, 3, objectVer);
        DBUtils.setShortOrNull(preparedStatement, 4, zlvl);
        return execute(preparedStatement);
    }
    ...
    public String getZType(Long zoid, Long zver) {
        ...
        PreparedStatement preparedStatement = DBStatements.getFunctionStatement(
                connection,
                "TIS.zobject",
                String.format("%s = %s", DBConstants.ZOID, zoid),
                String.format("%s = %s", DBConstants.ZVER, zver)
        );
        String ztype = null;
        if (preparedStatement != null) {
